...
public class DateUtil {
    ...
    public static String dateFormatToString(LocalDateTime date){
    ...
    public static Date addMonths(Date currentDate,int months){
        Calendar   calendar = new GregorianCalendar();
        calendar.setTime(currentDate);
        calendar.add(Calendar.MONTH,months); //把日期往后增加一天,整数  往后推,负数往前移动
        Date newDate=calendar.getTime(); //这个时间就是日期往后推一天的结果
        return  newDate;
    }
    public static Date addWeeks(Date currentDate,int weeks){
        return addDays(currentDate,weeks*7);
    }
